...
import pandas
from utils.nab.util import absoluteFilePaths, createPath
...
class Corpus(object):
  ...
  def getDataFile(self, dataset_path):
    """
    Collect all CSV data files from self.srcRoot directory.
    ...
    absolute_data_path=os.path.join(self.srcRoot, 'data', dataset_path)
    dataframe=pd.read_csv(absolute_data_path)
    return dataframe
