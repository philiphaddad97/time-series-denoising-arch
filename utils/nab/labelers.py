...
from utils.nab.util import (absoluteFilePaths,
                      getProbationPeriod,
                      strf,
                      strp,
                      deepmap,
...
class LabelCombiner(object):
  ...
  def applyWindows(self):
    ...
    for relativePath, anomalies in self.labelIndices.items():
      ...
      windows = []
      for a in anomalies:
        front = int(max(a - windowLength/2, 0))
        back = int(min(a + windowLength/2, length-1))
        # import pdb; pdb.set_trace()
        windowLimit = [strf(data["timestamp"][front]),strf(data["timestamp"][back])]
